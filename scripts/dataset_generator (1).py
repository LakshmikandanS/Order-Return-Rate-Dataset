...
# Add product-level stats
product_df["price_band"] = pd.cut(product_df["price"], bins=[0, 3000, 15000, 50000], labels=["low", "mid", "high"])
product_df["product_return_rate"] = np.round(np.random.uniform(0.1, 0.35, len(product_df)), 2)
product_df["category_return_rate"] = product_df.groupby("category")["product_return_rate"].transform("mean")
product_df["avg_rating"] = np.round(np.random.uniform(3.5, 4.5, len(product_df)), 1)
product_df["rating_variance"] = np.round(np.random.uniform(0.2, 1.0, len(product_df)), 2)
...
for i in range(NUM_CUSTOMERS):
    ...
    if seg == "zero":
        ...
        return_rate = random.uniform(0.05, 0.1)
    elif seg == "medium":
    ...
    customers.append([
        ...
        0,  # total_orders placeholder
        0,  # total_returns placeholder
        return_rate,
        random.randint(500, 20000),
        random.uniform(5, 30),
        random.randint(1, 60),
        random.choice(["Apparel", "Electronics", "Footwear", "Home Appliances"]),
...
for i in range(NUM_ORDERS):
    ...
    final_price = prod["price"] - discount_amt
    ...
    delay = np.random.choice([0, 1, 2, 3], p=[0.5, 0.25, 0.15, 0.1])
    ...
    if is_returned:
        ...
        elif prod["category"] in ["Apparel", "Footwear"]:
            reason = "SIZE_FIT_ISSUE"
        elif prod["category"] == "Electronics":
            reason = "QUALITY_DEFECT"
        elif prod["avg_rating"] < 3.8:
    ...
    logistics.append([
        order_id,
        shipping_mode,
        random.choice(["Delhivery", "BlueDart", "Ecom Express"]),
    ...
    returns.append([
        ...
        is_returned,
        reason,
        random.randint(1, 10) if is_returned else 0
    ])
...
logistics_df = pd.DataFrame(logistics, columns=[
    ...
    "delivery_attempts", "courier_delay_rate",
    "warehouse_processing_time"
])

returns_df = pd.DataFrame(returns, columns=[
    "order_id", "is_returned", "return_reason",
    "return_days_after_delivery"
])
...
return_counts = returns_df[returns_df["is_returned"] == 1]["order_id"]
...
customer_df["overall_return_rate"] = (
